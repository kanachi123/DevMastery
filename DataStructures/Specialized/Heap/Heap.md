...
## ***Куча - это специализированная структура данных,она описывается как дерево,которое имеет свойства кучи: B узел является потомком узла A(то есть A есть parent B или наоборот B является child of A) при том что f(A) $\geq$ f(B),где k(X) - ключ узла.***
